...
class Solver:
    ...
    def build_station(self, r: int, c: int) -> None:
        if self.field.build(STATION, r, c):
    ...
    def play(self) -> Result:
        while len(self.actions) < self.T:
            # 駅の配置
            r0, c0 = list(map(int, input().split()))
            r1, c1 = list(map(int, input().split()))

            # 建設計画をstack
            stack = []
            # 駅を建てる
            stack.append(('STATION', r0, c0))
            stack.append(('STATION', r1, c1))
            # 線路を配置して駅を接続する
                # r0 -> r1
            if r0 < r1:
                for r in range(r0 + 1, r1):
                    stack.append((RAIL_VERTICAL, r, c0))
                if c0 < c1:
                    stack.append((RAIL_RIGHT_UP, r1, c0))
                elif c0 > c1:
                    stack.append((RAIL_LEFT_UP, r1, c0))
            elif r0 > r1:
                for r in range(r0 - 1, r1, -1):
                    stack.append((RAIL_VERTICAL, r, c0))
                if c0 < c1:
                    stack.append((RAIL_RIGHT_DOWN, r1, c0))
                elif c0 > c1:
                    stack.append((RAIL_LEFT_DOWN, r1, c0))
            # c0 -> c1
            if c0 < c1:
                for c in range(c0 + 1, c1):
                    stack.append((RAIL_HORIZONTAL, r1, c))
            elif c0 > c1:
                for c in range(c0 - 1, c1, -1):
                    stack.append((RAIL_HORIZONTAL, r1, c))


            # お金がない時は待ちながら建設計画を進める
            while len(self.actions) < self.T and stack:

                action, r, c = stack.pop()

                if action == 'STATION':
                    while len(self.actions) < self.T and self.money < COST_STATION:
                        self.build_nothing()
                    ...
                    while len(self.actions) < self.T and self.money < COST_RAIL:
                        self.build_nothing()
                        self.money += self.income
                    self.build_rail(action, r, c)
                    self.calc_income()
                    self.money += self.income

            print('----cut---')
            print(Result(self.actions, self.money), flush=True)
            print('----cut---')
