...
fn main() {
    ...
    for i in 0..M {
        ...
        let district = build_roads_pq(n, candidates.clone(), 0);
    ...
    // 時間の許す限り２つのdistrictを入れ替えて再構築して改善するか試す
    while M >= 3 && start_time.elapsed().as_millis() < 170000 {
        // Gからランダムに連続する３グループを選ぶ
        ...
        let old_size = districts[selected[0]].size + districts[selected[1]].size + districts[selected[2]].size;

        let district1 = districts[selected[0]].clone();
        let district2 = districts[selected[1]].clone();
        let district3 = districts[selected[2]].clone();
        let mut candidates = HashSet::new();
        for city in district1.cities.iter() {
            candidates.insert(city.clone());
        }
        for city in district2.cities.iter() {
            candidates.insert(city.clone());
        }
        for city in district3.cities.iter() {
            candidates.insert(city.clone());
        }
        let new_district1 = build_roads_pq(district1.len(), candidates.clone(), rng.gen_range(0..8));
        for city in new_district1.cities.iter() {
            candidates.remove(city);
        }
        let new_district2 = build_roads_pq(district2.len(), candidates.clone(), rng.gen_range(0..8));
        for city in new_district2.cities.iter() {
            candidates.remove(city);
        }
        let new_district3 = build_roads_pq(district3.len(), candidates.clone(), rng.gen_range(0..8));
        let new_size = new_district1.size + new_district2.size + new_district3.size;
        if new_size < old_size {
            districts[selected[0]] = new_district1.clone();
            districts[selected[1]] = new_district2.clone();
            districts[selected[2]] = new_district3.clone();
        }
        // println!("# {} {} {} {} {}", selected[0], selected[1], selected[2], old_size, new_size);
    }
...
fn build_roads_pq(n: usize, candidates: HashSet<City>, rot: usize) -> District {
