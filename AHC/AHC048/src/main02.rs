...
fn main() {
    ...
    let mut recipe_map = vec![vec![]; recipes.len()];
    ...
    for i in 0..input.H {
        ...
        if kari[i].len() > 0 {
            ...
            recipe_map[recipe_id].retain(|&(_, idx)| idx != i);
        ...
        for (_, recipe_id) in top_recipes.iter() {
            // パレットにあるならそれを使う
            if recipe_map[*recipe_id].len() > 0 {
                let (pos, target): ((usize, usize), usize) = recipe_map[*recipe_id].pop().unwrap();
            ...
            if tubes.len() > 1 {
                ...
                for i in 0..tubes.len() - 1 {
                    ...
                    recipe_map[*recipe_id].push((positions[i+1].clone(), j));
