...
fn main() {
    ...
    let mut no_one_path = false;
    let mut mask = vec![false; N + M];
    let mut ans = vec![vec![]; M];
    let mut ct = 0;
    while !no_one_path && ct <= 2 && start_time.elapsed().as_millis() < 1000 {
        ct += 1;
        no_one_path = true;
        ...
        while !done {
            ...
            for i in 0..M {
                ...
                if let [s, v1, v2] = ans_[i].as_slice() {
                    if v1 == v2 {
                        if *v1 >= N && ans_[*v1 - N].is_empty() {
                            ans_[i].clear();
                            done = false; // 変更があったので再度チェック
                        }
                    } else {
                        if *v1 >= N && ans_[*v1 - N].is_empty() {
                            ans_[i] = [*s, *v2, *v2].to_vec();
                            done = false; // 変更があったので再度チェック
                        } else if *v2 >= N && ans_[*v2 - N].is_empty() {
                            ans_[i] = [*s, *v1, *v1].to_vec();
                            done = false; // 変更があったので再度チェック
                        }
                    }
                }
        ...
        // one pathが発生していないかチェック
        for i in 0..M {
            if ans_[i].is_empty() {
                continue;
            }
            if let [_, v1, v2] = ans_[i].as_slice() {
                if v1 == v2 && *v1 >= N {
                    // one pathが発生している
                    no_one_path = false;
                    mask[i + N] = true; // このソーターは無視
                    // break;
                }
            }
        }
    ...
    let mut visited = vec![false; N + M];
    let mut best_score = compute_score_details(&input, &output);

    // 時間の許す限りsorterの番号、v1とv2の入れ替えを試す
    let mut queue = VecDeque::new();
    queue.push_back(output.s);
    while let Some(u) = queue.pop_front() {
        ...
        visited[u] = true;
        for i in 0..K {
            let current_sorter = output.cs[u - N].0;
            let current_v1 = output.cs[u - N].1;
            let current_v2 = output.cs[u - N].2;
            ...
            if !visited[current_v2] {
                ...
                new_output.cs[u - N].1 = current_v2;
                new_output.cs[u - N].2 = current_v1;
                let score = compute_score_details(&input, &new_output);
                ...
                // sorterの番号を入れ替える
                if current_sorter == i {
                    continue; // 同じソーターはスキップ
                }
                let mut new_output = output.clone();
                new_output.cs[u - N].0 = i;
                let score = compute_score_details(&input, &new_output);
                if score.0 < best_score.0 {
                    best_score = score;
                    output = new_output;
