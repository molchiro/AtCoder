...
fn main() {
    ...
    let (mut best_score, mut best_output, best_sorters) = create_init_output(&input, &mut rng);
...
fn create_init_output(input: &Input, rng: &mut impl Rng) -> (i64, Output, Vec<usize>) {
    let mut best_sorters = vec![];
    // // 各ゴミの最良ソーターをまずは選ぶ
    // for i in 0..N {
    //     let mut ss = vec![];
    //     for j in 0..K {
    //         let mut sss = input.ps[j].clone();
    //         if sss[i] < 0.5 {
    //             for k in 0..N {
    //                 sss[k] = 1.0 - sss[k]; // 0.5未満なら反転
    //             }
    //         }
    //         let mut score = 0.0;
    //         for k in 0..N {
    //             if k == i {
    //                 score += (sss[k] - 0.5).powi(2);
    //             } else {
    //                 score -= (sss[k] - 0.5).powi(2);
    //             }
    //         }
    //         ss.push((score, j));
    //     }
    //     ss.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap());
    //     ss.reverse();
    //     println!("ss: {:?}", ss[0]);
    //     best_sorters.push(ss[0].1); // 最良のソーターを選ぶ
    // }
    // // 重複を削除
    // best_sorters.sort();
    // best_sorters.dedup();
    ...
    // 4回までの初期解を試す
    for (i, s) in graph[input.N + input.M].iter().enumerate() {
        let (score, output) = _create_init_output(input, &best_sorters, &graph, *s);
        if score < best_score {
            best_score = score;
            best_output = output;
        }
    ...
    (best_score, best_output, best_sorters)
}
...
pub fn compute_score_details(input: &Input, out: &Output) -> i64 {
