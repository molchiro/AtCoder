...
fn main() {
    ...
    for group in ans {
        let output = group.iter().map(|&x| x.to_string()).collect::<Vec<String>>().join(" ");
        println!("{}", output);
        // group内の点同士が最小全域木を構成するように辺選ぶ
        let mut edges = vec![];
        for i in 0..group.len() {
            for j in i + 1..group.len() {
                let d = culc_dist(input.range[group[i]], input.range[group[j]]);
                edges.push((d as usize, i, j));
            }
        }
        edges.sort();
