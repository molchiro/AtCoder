...
fn main() {
    ...
    let best_state = solve(input.clone(), recipe_map);
...
fn solve(input: Input, recipe_map: HashMap<Vec<usize>, ([f64; 3], Vec<usize>)>) -> State {
    let mut beam_width_1: usize = 5;
    const BEAM2: usize = 3;
    ...
    recipes.sort_by(|a, b| {
        a.color[0]
            .partial_cmp(&b.color[0])
            .unwrap()
            .then(a.color[1].partial_cmp(&b.color[1]).unwrap())
            .then(a.color[2].partial_cmp(&b.color[2]).unwrap())
    });
    // // 近い色を除外
    // let mut filtered_recipes = vec![];
    // filtered_recipes.push(recipes[0].clone());
    // for i in 1..recipes.len() {
    //     let prev = &filtered_recipes[filtered_recipes.len() - 1];
    //     let d = ((recipes[i].color[0] - prev.color[0]).powi(2)
    //         + (recipes[i].color[1] - prev.color[1]).powi(2)
    //         + (recipes[i].color[2] - prev.color[2]).powi(2))
    //         .sqrt();
    //     if d*10000.0 > input.D as f64 / 100.0 {
    //         filtered_recipes.push(recipes[i].clone());
    //     } else {
    //         if prev.tubes.len() > recipes[i].tubes.len() {
    //             // 近い色でチューブ数が少ないものを優先
    //             filtered_recipes.pop();
    //             filtered_recipes.push(recipes[i].clone());
    //         }

    //     }
    // }
    // // println!("{} recipes before filtering", recipes.len());
    // // println!("{} recipes after filtering", filtered_recipes.len());
    ...
    for i in 0..input.H {
        ...
        next_states.sort_by(|a, b| {
            (a.current_penalty_cumsum + a.future_penalty_cumsum /( (a.ct - a.delivered_n) as f64 + 0.01))
                .partial_cmp(&(b.current_penalty_cumsum + b.future_penalty_cumsum / ((b.ct - b.delivered_n) as f64 + 0.01) ))
                .unwrap()
        });
    ...
    // println!("current_penalty: {}", states[0].current_penalty_cumsum);
    // println!("future_penalty: {}", states[0].future_penalty_cumsum);
    // println!("score: {}", states[0].get_score(&input));
...
struct State {
    ...
    current_penalty_cumsum: f64,
    future_penalty_cumsum: f64,
}

impl State {
    fn new(wall_v: Vec<Vec<bool>>, wall_h: Vec<Vec<bool>>, input: Input, recipe_n: usize) -> Self {
        let vols = vec![0.0; input.N * input.N];
        ...
        Self {
            ...
            kari,
            ct: 0,
            future_penalty_cumsum: 0.0,
            current_penalty_cumsum: 0.0,
        }
    }
    ...
    fn apply(&mut self, input: &Input, action: Action) -> Result<(), String> {
        ...
        match action {
            Action::Add { i, j, k: _ } => {
                self.V += 1;
                self.vols[i * input.N + j] += 1.0;
            }
            Action::Deliver { i, j , color} => {
                let target = input.target[self.delivered_n];
                let d = ((color[0] - target[0]).powi(2)
                + (color[1] - target[1]).powi(2)
                + (color[2] - target[2]).powi(2))
            .sqrt();
                self.errors[self.delivered_n] = d;
                self.vols[i * input.N + j] = 0.0;
                self.delivered_n += 1;
                self.current_penalty_cumsum += d * 10000.0;
            }
            Action::Discard { i, j } => {
                self.vols[i * input.N + j] = 0.0;
            }
    ...
    fn deliver_reserved(&mut self, input: &Input, u: usize) -> Result<(), String> {
        ...
        let d = ((recipe.color[0] - input.target[u][0]).powi(2)
            + (recipe.color[1] - input.target[u][1]).powi(2)
            + (recipe.color[2] - input.target[u][2]).powi(2))
            .sqrt();
        self.future_penalty_cumsum -= d * 10000.0;
        Ok(())
    }
    ...
    fn deliver_from_palette(
        ...
        let d1 = ((recipe.color[0] - input.target[target][0]).powi(2)
            ...
            .sqrt();
        self.future_penalty_cumsum -= d1 * 10000.0;
    ...
    fn create_recipe_and_deliver(&mut self, input: &Input, recipe: &Recipe) -> Result<(), String> {
        ...
        // 絵の具を届ける
        let d = ((recipe.color[0] - input.target[self.delivered_n][0]).powi(2)
        + (recipe.color[1] - input.target[self.delivered_n][1]).powi(2)
        + (recipe.color[2] - input.target[self.delivered_n][2]).powi(2))
        .sqrt();
        ...
        if recipe.tubes.len() > 1 {
            ...
            for j in self.delivered_n..input.H {
            ...
            for i in 0..recipe.tubes.len() - 1 {
                ...
                if input.D < ((d - input.basis_d[target])*10000.0).round() as i32 {
                    ...
                    self.current_penalty_cumsum += input.D as f64;
                    self.ct -= 1;
                    continue;
                }
    ...
    fn find_n_size_well(
        ...
        for i in 0..input.N {
            for j in 0..input.N {
                ...
                if seen[i][j] || self.vols[i * input.N + j] > 0.0 {
                ...
                while stack.len() > 0 {
                    ...
                    if seen[x][y] || self.vols[i * input.N + j] > 0.0 {
                        continue; // すでに訪れたセル、または絵の具があるセルはスキップ
                    }
                    seen[x][y] = true;
                    positions.push((x, y));
                    ...
                    // 隣接するセルを探索 左>上>右>下 の優先度で探索
                    if x > 0 && self.vols[(x-1) * input.N + y] == 0.0 && !seen[x - 1][y] {
                        stack.push_back((x - 1, y));
                    }
                    if y > 0 && self.vols[x * input.N + y-1] == 0.0 && !seen[x][y - 1] {
                        stack.push_back((x, y - 1));
                    }
                    if x + 1 < input.N && self.vols[(x+1) * input.N + y] == 0.0 && !seen[x + 1][y] {
                        stack.push_back((x + 1, y));
                    }
                    if y + 1 < input.N && self.vols[x * input.N + y+1] == 0.0 && !seen[x][y + 1] {
                        stack.push_back((x, y + 1));
                    }
                }
