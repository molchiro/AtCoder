...
fn main() {
    ...
    let mut candidates_f = initial_state.get_candidates_for_fisrt();
    candidates_f.dedup_by(|a, b| {
        let (_, _, current_a, future_a) = a;
        let (_, _, current_b, future_b) = b;
        (current_a, future_a) == (current_b, future_b)
    });
    candidates_f.sort_by_key(|&(_, _, current, future)| (current, future));
    ...
    while !candidates_f.is_empty() &&start_time.elapsed().as_millis() < 2750 {
        let loop_start_time = time::Instant::now();

        let mut loop_initial_state = initial_state.clone();

        let (r_f, c_f, _, _) = candidates_f.pop().unwrap();
        println!("#first station: ({}, {})", r_f, c_f);
        ...
        while loop_start_time.elapsed().as_millis() < 200 {
            println!("# ct: {}", ct);
            let income_threshold = rng.gen_range(500..=2000);
            ...
            while state.operations.len() < 800 {
                ...
                if state.income < income_threshold {
                    // currentが正の物の中から上位を探索
                    candidates = candidates.into_iter().filter(|&(_, _, current, _, _)| current > 0).collect();
                    ...
                    candidates.dedup_by(|a, b| {
                        ...
                        (current_a, future_a, is_rail_a) == (current_b, future_b, is_rail_b)
                    });
    
                    // ２個目だけはお金が足りるところを絞り込む
                    if state.operations.len() == 1 {
                        candidates = candidates.into_iter().filter(|&(r, c, _current, _, _)| {
                            let cost = ((r_f as i64 - r as i64).abs() + (c_f as i64 - c as i64).abs()) * RAIL_COST + STATION_COST;
                            cost <= state.money
                        }).collect();
                    }
    
                    candidates = candidates.into_iter().take(20).collect();
...
impl State {
    ...
    fn get_candidates_for_fisrt(&self) -> Vec<(usize, usize, i64, i64)> {
        let candidates: Vec<(usize, usize, i64, i64)> = (0..self.n)
            .flat_map(|r| (0..self.n).map(move |c| (r, c)))
            .filter(|&(r, c)| self.grid_potential_first[r][c] > 0)
            .map(|(r, c)| (r, c, self.grid_potential_first[r][c], self.grid_potential_future[r][c]))
            .collect();
