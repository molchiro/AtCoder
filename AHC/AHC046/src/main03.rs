...
fn main() {
    ...
    while start_time.elapsed().as_millis() < 1950 {
        ...
        let actions_len = best_ans.actions.len();
        let alter_index = if elapsed < 800 {
            rng.gen_range(0..actions_len / 4)
        } else if elapsed < 1200 {
            rng.gen_range(actions_len / 4..actions_len / 2)
        } else if elapsed < 1600 {
            rng.gen_range(actions_len / 2..actions_len * 3 / 4)
        } else {
            rng.gen_range(actions_len * 3 / 4..actions_len)
        };

        let alter_direction = *DIRECTIONS.choose(&mut rng).unwrap();
        let mut new_ans = init_game.fast_clone();
