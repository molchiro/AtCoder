...
fn solve(input: Input, tube_number_cap: usize, recipe_map: &mut HashMap<Vec<usize>, ([f64; 3], Vec<usize>)>) -> State {
    ...
    for i in 0..input.H {
        ...
        for recipe_id in 0..recipes.len() {
            ...
            .sqrt();
            top_recipes.push(((d*10000.0) as i32, recipe_id));
        }
        top_recipes.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap());
        top_recipe_list.push(top_recipes);
    }
    ...
    for i in 0..input.H {
        ...
        let top_recipes = &mut top_recipe_list[i];
        if state.V > 990 {
            // ペナルティで補正をかける
            top_recipes.sort_by(|a, b| {
                let (d1, recipe_id1) = a;
                let v1 = d1 + (state.V + recipes[*recipe_id1].1.len() as i32 - 1000).max(0) * input.D;
                let (d2, recipe_id2) = b;
                let v2 = d2 + (state.V + recipes[*recipe_id2].1.len() as i32 - 1000).max(0) * input.D;
                v1.partial_cmp(&v2).unwrap()
            });
        }

        let mut top_10_recipes = top_recipes.iter().take(10).collect::<Vec<_>>();
 
        // 残りのターゲットに割り当てた時の平均エラーが小さい順にソート
        if i > 500 {
            top_10_recipes = top_10_recipes.iter().map(|&(d, recipe_id)| {
                // 余った絵の具を使うタイミングを決める
                ...
                for i in 0..tubes.len() - 1 {
                    if top_targets.len() <= i {
                        e += input.D as f64;
                    } else {
                        let (d, _) = top_targets[i];
                        e += d*10000.0;
                    }
                }
                (e as i32, *recipe_id)
        ...
        for (_, recipe_id) in top_recipes.iter() {
            ...
            if kari_map[*recipe_id].len() > 0 {
                ...
                // ct -= 1;
                let _ = state.apply(&input, Action::Deliver { i: pos.0, j: pos.1 });
                break;
            }
            ...
            if tubes.len() > 1 {
                ...
                for i in 0..tubes.len() - 1 {
                    if top_targets.len() <= i {
                        kari_map[*recipe_id].push((positions[i].clone(), 9999));
                    } else {
