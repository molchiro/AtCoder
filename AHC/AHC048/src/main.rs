...
fn main() {
    ...
    for n in 1..=iterations {
        create_recipe_map(input.clone(), n, &mut recipe_map);
    }
...
impl State {
    ...
    fn apply(&mut self, input: &Input, action: Action) -> Result<(), String> {
        ...
        match action {
            ...
            Action::Deliver { i, j , color} => {
                let target = input.target[self.delivered];
                self.errors[self.delivered] = ((color[0] - target[0]).powi(2)
    ...
    fn deliver_from_palette(
        ...
        let d2 = ((recipe.color[0] - input.target[self.delivered][0]).powi(2)
        + (recipe.color[1] - input.target[self.delivered][1]).powi(2)
        + (recipe.color[2] - input.target[self.delivered][2]).powi(2))
        .sqrt();
    ...
    fn create_recipe_and_deliver(&mut self, input: &Input, recipe: &Recipe) -> Result<(), String> {
        ...
        // 絵の具を届ける
        let d = ((recipe.color[0] - input.target[self.delivered][0]).powi(2)
        + (recipe.color[1] - input.target[self.delivered][1]).powi(2)
        + (recipe.color[2] - input.target[self.delivered][2]).powi(2))
        .sqrt();
        ...
        if recipe.tubes.len() > 1 {
            ...
            for j in self.delivered..input.H {
