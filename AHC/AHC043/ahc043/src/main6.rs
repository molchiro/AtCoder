...
fn main() {
    ...
    while !first_pairs.is_empty() &&start_time.elapsed().as_millis() < 2800 {
        ...
        while loop_start_time.elapsed().as_millis() < 150 {
            println!("# ct: {}", ct);
            let income_threshold = rng.gen_range(1000..=2501);
            ...
            while state.operations.len() < 800 {
                ...
                if state.income < income_threshold {
                    ...
                    candidates = candidates.into_iter().take(15).collect();
...
impl State {
    ...
    fn get_first_pair(&self) -> Vec<((usize, usize), (usize, usize), i64)> {
        ...
        for r_f in 0..self.n {
            for c_f in 0..self.n {
            for r_s in r_f..self.n {
                let c_s_start = if r_s == r_f { c_f } else { 0 };
                for c_s in c_s_start..self.n {
