...
fn main() {
    ...
    for i in 0..N {
        ...
        for j in i..N {
            ...
            if a.is_inside_of(&b) {
                ...
                if (roads.iter().filter(|&(u, v)| *u == j || *v == j).count() == 2) ^ (gx < GRID_SIZE/2) {
                ...
                if (roads.iter().filter(|&(u, v)| *u == j || *v == j).count() == 2) ^ (gy < GRID_SIZE/2) {
    ...
    for i in 0..M {
        ...
        let (selected, roads) = build_roads_pq(n, candidates.clone(), now_grid_address.1 % 2 == 0);

        ans.push(selected.iter().map(|city| city.id.to_string()).collect::<Vec<String>>().join(" "));
        if q_count < Q && 3 <= selected.len() && selected.len() <= L {
            println!("? {} {}", selected.len(), selected.iter().map(|city| city.id.to_string()).collect::<Vec<String>>().join(" "));
            input_interactive! {
                roads: [(usize, usize); selected.len() - 1],
            }
            q_count += 1;
            for (u, v) in roads {
                ans.push(format!("{} {}", u, v));
...
fn build_roads_pq(n: usize, candidates: HashSet<City>, reverse: bool) -> (HashSet<City>, Vec<(usize, usize)>) {
    ...
    if reverse {
        ...
        candidates.sort_by_key(|city| Reverse(city.estimated_x));
    }
    ...
    // println!("{:?}", pq);
    let mut selected: HashSet<City> = HashSet::new();
    let mut roads: Vec<(usize, usize)> = vec![];
    selected.insert(candidates[0].clone());
    while selected.len() < n {
        if let Some(Reverse((_, u, v))) = pq.pop() {
        if selected.contains(&candidates[v]) {
            continue;
        }
        selected.insert(candidates[v].clone());
        roads.push((candidates[u].id, candidates[v].id));
        for &(w,d) in &graph[v] {
            if selected.contains(&candidates[w]) {
            continue;
            }
...
impl City {
    ...
    fn is_inside_of(&self, other_city: &City) -> bool {
        other_city.lx <= self.lx  && self.rx <= other_city.rx && other_city.ly <= self.ly && self.ry <= other_city.ry
    }
