...
def solve(fld, oni_remaining):
    ...
    # 上辺
    for c in range(N):
        while not fld[0][c] == 'o' and (fld[0][c] == 'x' or fld[1][c] == 'x'):
            res.append(('U', c))
            oni_remaining -= U(fld, c)
    # 右辺
    for r in range(N):
        while not fld[r][-1] == 'o' and (fld[r][-1] == 'x' or fld[r][-2] == 'x'):
            res.append(('R', r))
            oni_remaining -= R(fld, r)
    # 下辺
    for c in range(N):
        while not fld[-1][c] == 'o' and (fld[-1][c] == 'x' or fld[-2][c] == 'x'):
            res.append(('D', c))
            oni_remaining -= D(fld, c)
    # 左辺
    for r in range(N):
        while not fld[r][0] == 'o' and (fld[r][0] == 'x' or fld[r][1] == 'x'):
            res.append(('L', r))
