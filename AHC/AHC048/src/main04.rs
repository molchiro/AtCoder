...
fn main() {
    ...
    let state = solve(input.clone(), 2, &mut recipe_map);
    if state.get_score() < best_state.get_score() {
        best_state = state;
    }
    let state = solve(input.clone(), 3, &mut recipe_map);
    if state.get_score() < best_state.get_score() {
        best_state = state;
    }
    if input.K < 21 {
        let state = solve(input.clone(), 4, &mut recipe_map);
        if state.get_score() < best_state.get_score() {
            best_state = state;
        }
    }
    if input.K < 18 {
        let state = solve(input.clone(), 5, &mut recipe_map);
        if state.get_score() < best_state.get_score() {
            best_state = state;
        }
    }
    if input.K < 14 {
        let state = solve(input.clone(), 6, &mut recipe_map);
        if state.get_score() < best_state.get_score() {
            best_state = state;
        }
    }
