...
fn main() {
    ...
    let mut best_state = State::new(&input);
    best_state.T = usize::MAX;
    ...
    while start_time.elapsed().as_millis() < 1950 {
        ...
        for (i, j) in cells {
            ...
            while state.pi != i || state.pj != j {
                // 今いる位置の荷物を持っても良いなら一つだけ持つ
                if state.d[state.pi][state.pj] - state.w[i][j] * (i as i32 + j as i32) > 0 {
                    // 遠いところにある荷物を先に捌きたいので、後勝ちにする
                    if f {
                        let mut stack = vec![];
                        // 荷物を持つ前まで戻る
                        while let Some(&last_action) = state.actions.last() {
                            if last_action != '1' {
                                stack.push(Some(last_action));
                                state.roleback().unwrap();
                            } else {
                                break;
                            }
                        }
                ...
                if candidates.is_empty() {
                    ...
                    state.apply(candidates[0]).unwrap();
                } else {
                    let choice = rng.gen_range(0..candidates.len());
                    state.apply(candidates[choice]).unwrap();
                }
            }
            
            // println!("now at ({}, {})", state.pi, state.pj);
            // 持つ
            state.apply('1').unwrap();
        ...
        if state.T < best_state.T {
            best_state = state;
        }
    }

    for c in best_state.actions.iter() {
        println!("{}", c);
    }
