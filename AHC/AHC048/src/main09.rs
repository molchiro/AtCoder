...
struct State {
    ...
    kari: Vec<Vec<((usize, usize), usize)>>,
...
impl State {
    ...
    fn deliver_reserved(&mut self, input: &Input, u: usize) -> Result<(), String> {
        let ((i1, j1), recipe_id) = self.kari[u].pop().unwrap();
        self.kari_map[recipe_id].retain(|&(_, idx)| idx != u);
        let res = self.apply(input, Action::Deliver { i: i1, j: j1 });
        if res.is_err() {
    ...
    fn deliver_from_palette(
        ...
        self.apply(input, Action::Deliver { i, j })
    }
    ...
    fn create_recipe_and_deliver(&mut self, input: &Input, recipe: &Recipe) -> Result<(), String> {
        ...
        // 絵の具を混ぜる
        for tube in recipe.tubes.iter() {
            let k = *tube;
            let res = self.apply(&input, Action::Add { i: positions[0].0, j: positions[0].1, k });
            if res.is_err() {
        ...
        + (recipe.color[2] - input.target[self.delivered.len()][2]).powi(2))
        .sqrt();
        let res = self.apply(&input, Action::Deliver { i: positions[recipe.tubes.len()-1].0, j: positions[recipe.tubes.len()-1].1 });
        if res.is_err() {
            panic!("絵の具を届けることができませんでした: {:?}", res);
        }
        ...
        if recipe.tubes.len() > 1 {
            ...
            for i in 0..recipe.tubes.len() - 1 {
                ...
                self.kari[target].push((positions[i].clone(), recipe.recipe_id));
                self.penalty_cumsum += d * 10000.0;
            }
        }
        Ok(())
