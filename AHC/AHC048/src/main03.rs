...
fn main() {
    ...
    let mut recipe_map: HashMap<(usize, usize, usize), ([f64; 3], Vec<usize>)> = HashMap::new();
    // 1種類の絵の具
    for i in 0..input.K {
        recipe_map.insert((i, 999, 999), (input.own[i], vec![i]));
    }
    // 2種類の絵の具
    for i in 0..input.K {
        for j in i + 1..input.K {
            recipe_map.insert((i, j, 999),(mix(1.0, input.own[i], 1.0, input.own[j]), vec![i, j]));
        }
    }
    // 3種類の絵の具
    for i in 0..input.K {
        for j in 0..input.K {
            for k in 0..input.K {
                ...
                recipe_map.insert((x, y, z),(mixed, vec![i, j, k]));
            }
        }
    }
    let mut recipes = vec![];
        ...
        for (_, recipe_id) in top_recipes.iter() {
            ...
            if tubes.len() > 1 {
                ...
                for i in 0..tubes.len() - 1 {
                    ...
                    kari_map[*recipe_id].push((positions[i+1].clone(), j));
                    kari[j].push((positions[i+1].clone(), *recipe_id));
                }
            }
