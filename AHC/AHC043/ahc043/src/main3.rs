...
fn main() {
    ...
    while start_time.elapsed().as_millis() < 2850 {
        println!("# ct: {}", ct);
        let t_th_1 = rng.gen_range(2..=200);
        let t_th_2 = rng.gen_range(t_th_1..=400);
        
        let mut state = initial_state.clone();
        
        while state.operations.len() < 800 {
            let mut candidates = state.get_candidates();
            // 初期は短期的な収入優先、中期は将来的な収入優先、後期は短期的なの収入優先
            if state.operations.len() < t_th_1 {
                candidates.sort_by_key(|&(_, _, current, future, is_rail)| (current, is_rail, future));
            } else if state.operations.len() < t_th_2  {
                candidates.sort_by_key(|&(_, _, current, future, is_rail)| (future, is_rail, current));
            } else {
                candidates.sort_by_key(|&(_, _, current, future, is_rail)| (current, is_rail, future));
            }
