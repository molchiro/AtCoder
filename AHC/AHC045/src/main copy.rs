...
fn main() {
        ...
        for j in i..N {
            ...
            if a.is_inside_of(&b) {
                ...
                if (roads.iter().filter(|&(u, v)| *u == j || *v == j).count() == 2)
                    ^ (gx < GRID_SIZE / 2)
                {
                    cities[j].update(b.lx, a.lx, b.ly, b.ry);
                } else {
                    cities[j].update(a.rx, b.rx, b.ly, b.ry);
                }
                ...
                if (roads.iter().filter(|&(u, v)| *u == j || *v == j).count() == 2)
                    ^ (gy < GRID_SIZE / 2)
                {
                    cities[j].update(b.lx, b.rx, b.ly, a.ly);
                } else {
                    cities[j].update(b.lx, b.rx, a.ry, b.ry);
                }
                // a = cities[j].clone();
            }
        }
    ...
    for i in 0..M {
        ...
        let (selected, roads) = build_roads_pq(n, candidates.clone());
        ...
        if 3 <= selected.len() && selected.len() <= L {
            println!(
                "? {} {}",
                selected.len(),
                selected
...
fn build_roads_pq(n: usize, candidates: HashSet<City>) -> (HashSet<City>, Vec<(usize, usize)>) {
