...
fn main() {
    ...
    let mut best_state = search(input.clone(), start_time, &mut rng);

    while start_time.elapsed().as_millis() < 1950 {
        let new_state = search(input.clone(), start_time, &mut rng);
        if new_state.true_score() > best_state.true_score() {
            // eprintln!("score: {} -> {}", best_state.true_score(), new_state.true_score());
...
fn search(input: Input, start_time: std::time::Instant, rng: &mut rand_chacha::ChaCha20Rng) -> State {
    let mut cs = vec![];
    ...
    let state = State::new(input, cs);

    let mut best_state = greedy(&state);
    let mut candidates = vec![];
    candidates.push(state);
    while candidates.len() > 0 && start_time.elapsed().as_millis() < 1950 {
        let mut new_candidates = vec![];
        for candidate in candidates {
            for i in 0..10 {
                let one_step_forward = candidate.act(i);
                let res = greedy(&one_step_forward);
                if res.true_score() > best_state.true_score() {
                    best_state = res.clone();
                }
                if res.rem > 0 {
        ...
        new_candidates.sort_by_key(|s| -s.true_score());
        candidates = new_candidates.into_iter().take(20000).collect();
    }
    best_state
}

fn greedy(state: &State) -> State {
