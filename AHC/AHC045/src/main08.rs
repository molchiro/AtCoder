...
fn main() {
    ...
    for i in 0..N {
        ...
        for j in i..N {
            ...
            if a.is_inside_of(&b) {
                // gridからcities[j]を削除
                let gx_j = (cities[j].estimated_x * GRID_SIZE) / x_max;
                let gy_j = (cities[j].estimated_y * GRID_SIZE) / y_max;
                grid[gx_j][gy_j].retain(|&x| x != j);
                q_count += 2;
                ...
                if (roads.iter().filter(|&(u, v)| *u == j || *v == j).count() == 2)
                    ^ (gx < GRID_SIZE / 2)
                {
                    cities[j].update(b.lx, a.estimated_x, b.ly, b.ry);
                } else {
                    cities[j].update(a.estimated_x, b.rx, b.ly, b.ry);
                }
                ...
                if (roads.iter().filter(|&(u, v)| *u == j || *v == j).count() == 2)
                    ^ (gy < GRID_SIZE / 2)
                {
                    cities[j].update(b.lx, b.rx, b.ly, a.estimated_y);
                } else {
                    cities[j].update(b.lx, b.rx, a.estimated_y, b.ry);
                }

                // gridに更新後のcities[j]を追加
                let gx_j = (cities[j].estimated_x * GRID_SIZE) / x_max;
                let gy_j = (cities[j].estimated_y * GRID_SIZE) / y_max;
                grid[gx_j][gy_j].push(j);
            }
        }
    }
    ...
    while start_time.elapsed().as_millis() < 1800 {
        let districts = solve_rnd(&mut cities, &grid, &G, M, &mut rng);
        let mut score = 0.0;
        for i in 0..M {
...
fn solve_rnd(
    ...
    for i in 0..M {
        ...
        // ある程度余裕を持って取り出す
        while candidates.len() < n + rng.gen_range(50..150) && now_grid < GRID_SIZE * 2 {
