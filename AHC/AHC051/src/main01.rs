...
fn main() {
    ...
    while let Some(u) = queue.pop_front() {
        ...
        let mut graph_u = graph[u].clone();
        // 中心（5000, 5000）から遠い順にソート
        // graph_u.sort_unstable_by_key(|&v| {
        //     let (x, y) = pos[v];
        //     (x - 5000).pow(2) + (y - 5000).pow(2)
        // });
        // graph_u.reverse(); // 遠い順にするために逆順に
...
/// ドロネー三角分割の辺集合を返す（整数版）
pub fn delaunay_edges(points: &Vec<(i64, i64)>) -> Vec<Edge> {
