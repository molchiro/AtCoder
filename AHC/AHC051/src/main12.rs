...
fn main() {
    ...
    // 時間が大量に余っていれば繰り返す
    while start_time.elapsed().as_millis() < TIME_LIMIT*3 / 5 {
        let (score, output, _) = create_init_output(&input, &mut rng);
        if score < best_score {
            best_score = score;
            best_output = output;
